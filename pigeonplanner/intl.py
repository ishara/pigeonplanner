# -*- coding: utf-8 -*-
#
# This file was taken from elib.intl, modified for Pigeon Planner
...
def _install(domain, localedir, language):
    ...
    if sys.platform == 'win32' or sys.platform == 'nt':
        ...
        if 'LANG' not in os.environ:
            ...
            os.environ['LANG'] = output.strip() + '.UTF-8'
...
def install(domain, localedir, language):
    '''
    :param domain: translation domain
    :param localedir: locale directory
    ...
    try:
        ...
        if language not in ('', 'en_US.UTF-8'):
            logger.debug("Couldn't set language for '%s': %s" % (language, exc))
