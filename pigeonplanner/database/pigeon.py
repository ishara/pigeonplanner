...
def update_result_as_race(date, racepoint, sector, wind, weather):
    session.cursor.execute("UPDATE Results SET sector=?, wind=?, weather=? WHERE date=? AND point=?", (sector, wind, weather, date, racepoint))
    session.connection.commit()
