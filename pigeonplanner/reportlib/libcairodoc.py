...
from . import utils
from .backend import CairoBackend
from .utils import resize_to_buffer
from .utils import SystemFonts
...
class GtkDocPictureDraw(GtkDocBaseElement):
    """Implement an image for draw reports.
    """
    _type = 'IMAGE_DRAW'
    _allowed_children = []

    def __init__(self, filename, x, y, width, height, xalign, yalign, crop=None):
    ...
    def draw(self, cr, layout, width, dpi_x, dpi_y):
        ...
        if self._xalign == 'right':
            x = img_x - pixbuf_width
        elif self._xalign == 'center':
            x = img_x - pixbuf_width / 2
        else:
            x = img_x

        if self._yalign == 'bottom':
            y = img_y - pixbuf_height
        elif self._yalign == 'center':
            y = img_y - pixbuf_height / 2
        else:
            y = img_y
        ...
        # gcr.set_source_pixbuf(pixbuf, x, y)
        # gcr.paint()
        cr.restore()
...
class GtkDocFrame(GtkDocBaseElement):
    ...
    """
    _type = 'FRAME'
    _allowed_children = ['LINE', 'POLYGON', 'BOX', 'TEXT', 'IMAGE_DRAW']
...
class CairoDoc(BaseDoc, TextDoc, DrawDoc):
    ...
    def open(self, filename, parent=None):
        fe = filename.split('.')
        if len(fe) == 1:
...
"""Mismatch between selected extension %(ext)s and actual format.
 Writing to %(filename)s in format %(impliedext)s.""" %
    ...
    def close(self, print_action=None):
        self.run(print_action)
...
links (like ODF) and write PDF from that format.
    ...
    def draw_image(self, name, x, y, x_cm, y_cm, alt='', style_name=None,
                         crop=None, xalign='left', yalign='top'):

        new_image = GtkDocPictureDraw(name, x, y, x_cm, y_cm, xalign, yalign, crop=crop)
        self._active_element.add_child(new_image)
